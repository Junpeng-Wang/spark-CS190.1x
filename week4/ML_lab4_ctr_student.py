...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    indexList = list()
    valueList = list()
    for object in rawFeats:
        indexList.append(OHEDict[object])
        valueList.append(1.)
    indexList.sort()   
    return SparseVector(numOHEFeats, indexList, valueList)
...
def parsePoint(point):
    ...
    for i in range(len(tokens)):
        if i>0:
            featureList.append((id, tokens[i]))
            id = id + 1
    return featureList
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    """
    featureList = parsePoint(point)
    ctrOneOHEFeat = oneHotEncoding(featureList, OHEDict,
                                 numOHEFeats)
    fList = point.split(',')
    label = fList[0]
    return  LabeledPoint(label, ctrOneOHEFeat)
...
# TODO: Replace <FILL IN> with appropriate code
model0 = LogisticRegressionWithSGD.train(OHETrainData, iterations=numIters, step=stepSize, miniBatchFraction=1.0, initialWeights=None, regParam=1e-6, regType='l2', intercept=True)
