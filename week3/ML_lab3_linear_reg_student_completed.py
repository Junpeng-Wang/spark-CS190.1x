...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda (x1,x2): squaredError(x1,x2)).mean())
...
def gradientSummand(weights, lp):
    ...
    """
    x = lp.features
    return np.dot((np.dot(weights.T, x) - lp.label),x)
...
def getLabeledPrediction(weights, observation):
    ...
    """
    return (observation.label, np.dot(weights.T, observation.features))
...
def twoWayInteractions(lp):
    ...
    return LabeledPoint(lp.label, np.hstack((lp.features, 
                                             [x*y for (x,y) in list(itertools.product(np.array(lp.features), 
                                                                                      np.array(lp.features))
                                                                   )])))
